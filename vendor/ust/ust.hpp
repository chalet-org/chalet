#ifndef UNIVERSAL_STACK_TRACE_HPP
#define UNIVERSAL_STACK_TRACE_HPP
...
inline StackTrace generate()
{
	...
	if (result)
	{
		...
		{
			BOOL result =
				StackWalk64(image, process, thread, &stackframe, &context, NULL, SymFunctionTableAccess64, SymGetModuleBase64, NULL);
			...
			if (SymGetSymFromAddr64(process, stackframe.AddrPC.Offset, &displacement, pSymbol))
			{
			...
			std::string sourceFileName;
			int lineNumber = -1;
			if (SymGetLineFromAddr64(process, stackframe.AddrPC.Offset, &displacement32, &theLine))
			{
			...
			stackTrace.push_back(StackTraceEntry(
				i, addressToString(stackframe.AddrPC.Offset), binaryFileName, functionName, sourceFileName, lineNumber));
		}
	}
...
inline StackTrace generate()
{
	...
	for (unsigned short a = 0; a < numFrames; a++)
	{
		...
		GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS, (const char*)stack[a], &moduleHandle);
		std::string fileName(4096, '\0');
		auto fileNameSize = GetModuleFileNameA(moduleHandle, &fileName[0], fileName.size());
	...
	for (int a = 0; a < numFrames; ++a)
	{
		...
		// [0x55f1278af96e]
		auto parenStart = line2.find("(");
		auto parenEnd = line2.find(")");
		fileName = line2.substr(0, parenStart);
		// Convert filename to canonical path
		char buf[PATH_MAX];
		::realpath(fileName.c_str(), buf);
		fileName = std::string(buf);
		functionName = line2.substr(parenStart + 1, parenEnd - (parenStart + 1));
		...
		{
			int status = 0;
			auto demangledFunctionName = abi::__cxa_demangle(functionName.data(), 0, 0, &status);
...
#endif // UNIVERSAL_STACK_TRACE_HPP