...
namespace subprocess {
    ...
    Popen ProcessBuilder::run_command(const CommandLine& inCommand) {
        ...
        pid_t pid;
        cstring_vector args;
        args.reserve(inCommand.size()+1);
        args.push_back(program);
        for(size_t i = 1; i < inCommand.size(); ++i) {
            args.push_back(inCommand[i]);
        }
        args.push_back(nullptr);
        char** localenv = environ;
        ...
        attributes_raii.setflags(flags);
        {
            ...
            */
            ...
            int ret = posix_spawn(&pid, args[0], actions.get(), &attributes, &args[0], localenv);
            if(ret != 0)
