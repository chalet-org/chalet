...
ICompileToolchain::ICompileToolchain(const BuildState& inState, const ProjectTarget& inProject, const CompilerConfig& inConfig) :
	...
	m_config(inConfig)
{
	m_quotePaths = m_state.strategy() != StrategyType::Native;

	m_isMakefile = m_state.strategy() == StrategyType::Makefile;
	m_isNinja = m_state.strategy() == StrategyType::Ninja;
	m_isNative = m_state.strategy() == StrategyType::Native;
}
