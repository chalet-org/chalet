...
bool CompileToolchainGNU::initialize()
{
	...
	if (m_project.usesPch())
	{
		...
		if (!Commands::pathExists(pchIntermediate))
		{
			auto pragmaId = type() == ToolchainType::GNU ? "GCC" : "clang";
...
#pragma {pragmaId} system_header
	...
	#include "{pchAbsolute}"
			...
			if (!Commands::createFileWithContents(pchIntermediate, contents))
				return false;
		}

		std::string pchSourceIntermediate = fmt::format("{}.cxx", pchIntermediate);
		if (!Commands::pathExists(pchSourceIntermediate))
		{
			auto pchIncludeAbsolute = Commands::getAbsolutePath(pchIntermediate);
			auto sourceContents = fmt::format(R"cpp(// Generated by Chalet
...
#include "linked.h"
			...
			if (!Commands::createFileWithContents(pchSourceIntermediate, sourceContents))
				return false;
		}
	}
