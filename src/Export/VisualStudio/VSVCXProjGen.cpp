		...
		if (projectPtr != nullptr)
		{
			...
			if (m_adapters.find(config) != m_adapters.end())
			{
				...
				for (auto& header : headerFiles)
				{
					...
					auto file = Commands::getCanonicalPath(header);
				...
				{
					auto file = Commands::getCanonicalPath(group->sourceFile);
					if (!Commands::pathExists(file))
				...
				{
					auto file = Commands::getCanonicalPath(manifest.first);
					if (Commands::pathExists(file))
						manifest.first = file;
				...
				if (!icon.first.empty())
				{
					auto file = Commands::getCanonicalPath(icon.first);
					if (Commands::pathExists(file))
						icon.first = file;
...
{
	auto& firstState = *m_states.front();
	auto inputFile = Commands::getCanonicalPath(firstState.inputs.inputFile());
