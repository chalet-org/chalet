		...
		for (const auto& [target, pbxGroup] : groups)
		{
			...
			if (isSource || isAppBundle)
			{
				...
				if (isAppBundle)
				{
					...
					node[key]["buildPhases"].push_back(copyFiles);

					// auto embedLibraries = getSectionKeyForTarget("Embed Libraries", target);
					// node[key]["buildPhases"].push_back(embedLibraries);
				}
			}
		}
	}
	...
	// XCBuildConfiguration
	{
		...
		for (auto& state : m_states)
		{
			...
			for (auto& target : state->distribution)
			{
				...
				{
					auto name = target->name();
					if (groups.find(fmt::format("{}.app", name)) != groups.end())
						name = fmt::format("{}.app", name);
...
StringList XcodePBXProjGen::getLinkerOptions(const BuildState& inState, const SourceTarget& inTarget) const
{
	...
	for (auto& link : links)
	{
		if (Commands::pathExists(link))
			ret.push_back(Commands::getCanonicalPath(link));
		else
	...
	// Apple Framework Options
	{
		...
		{
			ret.emplace_back(fmt::format("-F{}", Commands::getCanonicalPath(path)));
		}
