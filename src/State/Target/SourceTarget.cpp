...
bool SourceTarget::initializeUnityBuild()
{
	...
	if (m_unityBuild && !excludeInStrategies)
	{
		...
		if (cxxExt.empty())
		{
			Diagnostic::error("Unity build requested in a non-Cxx project.");
			return false;
		}
		...
		auto sourceFile = fmt::format("{}/{}_unity.{}", m_state.paths.intermediateDir(*this), this->name(), cxxExt);

		{
			...
			auto contents = fmt::format("// Unity build file generated by Chalet\n\n{}", includes);
			...
			if (Commands::pathExists(sourceFile))
			{
				existingContents = Commands::getFileContents(sourceFile);
				if (!existingContents.empty())
					existingContents.pop_back(); // last '\n'

				generateFile = existingContents.empty() || !String::equals(contents, existingContents);
			}

			if (generateFile)
			{
				if (!Commands::createFileWithContents(sourceFile, contents))
				{
