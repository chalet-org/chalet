...
bool remove(const std::string& inPath, const bool inShowError = true);
...
bool copyIfDoesNotExistWithoutPrintingWorkingDirectory(const std::string& inFrom, const std::string& inTo, const std::string& cwd);
...
bool readAndReplace(const std::string& inFile, const std::function<void(std::string&)>& onReplace);
